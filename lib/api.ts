...
export async function startCheckout(paymentRef: string, plan: PlanOption = 1): Promise<CheckoutResponse> {
  const body: Record<string, string | number> = { paymentRef, plan };
  if (Platform.OS === 'web') {
...
export async function generateGame(imageUri: string, playToken?: string): Promise<GenerateResponse> {
  const base64 = await readImageAsBase64(imageUri);

  const body: Record<string, string> = { image: base64 };
  if (playToken) body.playToken = playToken;
